...
data=rt
group <- c(rep("COVID_19",100),rep("no_COVID_19",26)) 
group <- factor(group,levels = c("no_COVID_19","COVID_19"))
...
write.table(diffgene,file = "diff_MF.txt",quote = F,sep = "\t")
...
for_label <- df %>% 
  filter(logFC >0.7& adj.P.Val< 0.05)
p1 +
  ...
  theme(axis.text.x = element_text(size = 15,angle = 45,vjust = 0.5,hjust = 0.5))+
  theme(axis.title.x =  element_text(size = 15,vjust = 0.5,hjust = 0.5))+
  theme(axis.text.y = element_text(size = 15,angle = 45,vjust = 0.5,hjust = 0.5))+
  theme(legend.position = "NA")
ggsave("MF_volcano.pdf", height = 8, width = 8)
